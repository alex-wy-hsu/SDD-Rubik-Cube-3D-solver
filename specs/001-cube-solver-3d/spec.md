...
2. **給定** 用戶正在查看方塊，**當** 用戶點擊任一面片，**則** 該面片所屬的層被柔和高亮顯示
...
應用程式啟動時，系統從已解狀態開始，執行預設 25 次（可配置為 N 次）隨機合法移動來生成打亂狀態。系統記錄並顯示打亂種子（scramble seed），以便重現相同的打亂。用戶可以點擊「重新打亂」按鈕生成新的打亂狀態。
...
1. **給定** 應用程式剛啟動，**當** 3D 場景載入完成，**則** 方塊顯示為已打亂狀態（N=25 次移動）
...
- 當用戶在解題動畫播放時關閉應用程式會發生什麼？系統應安全終止並保存當前狀態（或不保存，明確定義行為）
- 當 SLM 模型載入失敗或回應超時時如何處理？系統必須偵測並自動降級到演算法版本
- 當用戶連續快速點擊多個箭頭會發生什麼？系統應該排隊處理或忽略後續點擊直到當前動畫完成
- 當打亂次數 N 設置為極大值（例如 1000）時，初始化時間是否可接受？
...
- **FR-003**：用戶必須能夠點擊方塊上的任一面片，系統高亮顯示該面片所屬的層（3×3 個面片）
...
**打亂與初始化**
- **FR-007**：系統必須在啟動時從已解狀態開始，執行 N 次（預設 25 次）隨機合法移動生成打亂狀態
...
- **FR-015**：系統必須提供「停止」功能，允許用戶中斷解題動畫並保持當前方塊狀態
- **FR-016**：系統必須驗證解法執行後方塊處於已解狀態（所有六個面各自單色）

**SLM 求解**
- **FR-017**：系統必須整合一個嵌入式小型語言模型（SLM），能夠接收方塊狀態並生成解法步驟序列
...
**狀態管理與驗證**
- **FR-023**：系統必須維護方塊的內部狀態表示，追蹤每個面片的顏色和位置
- **FR-024**：系統必須在每次移動後驗證方塊狀態仍然合法（不產生不可能的配置）
- **FR-025**：系統必須支援狀態序列化，以便保存、載入或傳輸方塊配置
- **FR-026**：系統必須提供狀態檢查功能，判斷當前方塊是否處於已解狀態
...
- 用戶操作必須提供即時反饋（高亮效果、箭頭顯示立即回應）
- 錯誤訊息必須清晰且可操作（例如「SLM 求解失敗，已切換到演算法版本」）
- UI 必須支援鍵盤導航（可選）
...
- **記憶體使用**：應用程式記憶體佔用必須保持在 200MB 以下
- **初始載入**：應用程式啟動到首次可互動必須在 <3 秒內完成
